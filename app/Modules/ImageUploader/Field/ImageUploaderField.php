<?php
/**
 * App > Modules > Image Uploader > Field > Image Uploader Field.
 *
 * @since   1.0.0
 *
 * @version 1.0.0
...
namespace Handy\Modules\ImageUploader\Field;

use Handy\Core\Setting;
use Handy\Inc\Helper;
use Handy\Inc\Validator;
use Handy\Modules\ImageUploader\Control\ImageUploaderControl;

defined( 'ABSPATH' ) || exit;

/**
 * The `ImageUploaderField` class contains the settings,
 * sanitization and validation.
 *
 * @since 1.0.0
 */
final class ImageUploaderField extends Setting {
    ...
    private function allowed_extensions() {
        return array( 'jpg', 'jpeg', 'png', 'gif', 'ico', 'svg', 'webp' );
    }
     ...
     * @param  array  $args      Contains the necessary arguments to render image uploader control.
     * $args = [
    ...
    public function render( $customize, $args = array() ) {
        ...
        if ( $validated && $config ) {
            $this->setting( 'image_uploader', $customize, $validated );
            $customize->add_control( new ImageUploaderControl( $customize, $config['settings'], $config ) );
        }
    }
}
